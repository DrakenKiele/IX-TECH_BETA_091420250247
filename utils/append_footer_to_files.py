...
EXTENSIONS = {'.py', '.js', '.css', '.html', '.json'}
...
def format_footer(ext):
    if ext == '.py':
        return '\n' + '\n'.join(f'# {line}' if line else '#' for line in FOOTER_LINES) + '\n'
    elif ext in {'.js', '.css'}:
        # Use block comment for JS/CSS
        content = '\n'.join(f' * {line}' if line else ' *' for line in FOOTER_LINES)
        return '\n/*\n' + content + '\n */\n'
    elif ext == '.html':
        content = '\n'.join(f'  {line}' if line else '' for line in FOOTER_LINES)
        ...
        # JSON does not support comments; skip footer
        return ''
    else:
        return ''


def append_footer_to_files(root):
    for dirpath, dirs, files in os.walk(root):
        ...
        for file in files:
            ...
            if ext in EXTENSIONS:
                ...
                footer = format_footer(ext)
                if not footer:
                    continue
                with open(full_path, 'a', encoding='utf-8') as f:
                    f.write(footer)
                print(f"Appended footer to: {full_path}")

if __name__ == '__main__':
    append_footer_to_files('.')
