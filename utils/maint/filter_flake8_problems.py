# BREADCRUMB: [Project/Module] > filter_flake8_problems.py
...
# -----------------------------------------------------------------------------
# File: filter_flake8_problems.py
...
"""
filter_flake8_problems.py

Filters out non-critical flake8 problems from a flake8 output file.
Edit the IGNORED_CODES list to add/remove codes you want to ignore.
Usage:
    python filter_flake8_problems.py flake8_output.txt > filtered_output.txt
"""
import sys
import re

# List of flake8 codes to ignore (non-critical)
IGNORED_CODES = [
    'E501',  # line too long
...
pattern = re.compile(r'\b(' + '|'.join(IGNORED_CODES) + r')\b')

if len(sys.argv) < 2:
    print("Usage: python filter_flake8_problems.py flake8_output.txt > filtered_output.txt", file=sys.stderr)
    sys.exit(1)
